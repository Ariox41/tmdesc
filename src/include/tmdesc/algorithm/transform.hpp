...
public:
    ...
    constexpr decltype(auto) operator()(Tuple&& t, Fn&& fn) const
        noexcept(noexcept(transform(std::declval<Tuple>(), std::declval<Fn>(), index_sequence_for_tuple<Tuple>{}))) {
        return transform(std::forward<Tuple>(t), std::forward<Fn>(fn), index_sequence_for_tuple<Tuple>{});
    }
};

template <template <class...> class Consumer> constexpr tuple_transform_t<Consumer> tuple_transform_to{};
...
public:
    ...
    constexpr decltype(auto) operator()(Tuple1&& t1, Tuple2&& t2, Fn&& fn) const
        noexcept(noexcept(tuple_unpack(transform(std::declval<Tuple1>(), std::declval<Tuple2>(), std::declval<Fn>(),
...
template <template <class...> class Consumer> constexpr tuple_transform2_t<Consumer> tuple_transform2_to{};
